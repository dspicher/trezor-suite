...
// differences:
// - `fromBase58Check` method is using additional "network" param and checking multibyte version (Zcash and Decred support).
// - `toBase58Check` method is using additional "network" param and bs58check.encodeMultibytePayload instead of bs58check.encode.
...
export function fromBase58Check(address: string, network = BITCOIN_NETWORK): Base58CheckResult {
    const payload = bs58check.decode(address, network);
...
export function toBase58Check(hash: Buffer, version: number, network = BITCOIN_NETWORK): string {
    ...
    return bs58check.encodeMultibytePayload(hash, version, network);
}
