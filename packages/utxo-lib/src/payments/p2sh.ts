...
// differences:
// - using extended bs58check (bs58check.decode with network param)
// - using bs58check.encodeMultibytePayload instead of bs58check.encode
...
export function p2sh(a: Payment, opts?: PaymentOpts): Payment {
    ...
    const _address = lazy.value(() => {
        const payload = bs58check.decode(a.address!, network);
    ...
    lazy.prop(o, 'address', () => {
        if (!o.hash) return;
        return bs58check.encodeMultibytePayload(o.hash, network!.scriptHash, network);
    });
    lazy.prop(o, 'hash', () => {
